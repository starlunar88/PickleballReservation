...
function drawParticipationBarChart(data) {
    ...
    // 바 차트 그리기
    data.forEach((item, index) => {
        const barHeight = (item.total / yMax) * chartHeight;
...
async function renderRecords(matches) {
    ...
    matches.forEach(match => {
        ...
        if (match.gameStartTime && match.gameEndTime) {
            // 저장된 시간이 있으면 사용 (시간당 8경기 간격으로 계산된 시간)
                ...
                <div class="timeline-item ${statusClass}" data-time-slot="${slotKey}" data-date="${targetDate}">
                    ...
                    <div class="timeline-actions">
                        ${(() => {
                            ...
                            // 예약하기/취소하기 버튼 (항상 표시)
                            if (isClosed) {
                                // 마감 후에는 예약하기와 예약 취소 모두 비활성화
                                if (userReservation) {
                                    buttons += `<button class="timeline-cancel-btn" disabled style="opacity: 0.5;">
                                                취소하기
                                            </button>`;
                                } else {
...
async function recordGameResult(teamId, gameResult) {
    try {
        ...
        const gameData = {
            ...
            gameStartTime: gameResult.gameStartTime || null, // 시간당 8경기 간격으로 계산된 시작 시간
            gameEndTime: gameResult.gameEndTime || null,     // 시간당 8경기 간격으로 계산된 종료 시간
...
async function generateMatchSchedule(date, timeSlot, teamMode = 'random') {
    try {
        ...
        if (teamMode === 'balanced') {
            // 새로운 밸런스 모드 사용
            const result = buildPickleballBalancedSchedule(playersToUse, courtCount);
...
// 그룹 기반 밸런스 로테이션 스케줄 생성 함수
// DUPR 점수 순으로 정렬 후 상위/하위 그룹으로 나누어 밸런스 유지하며 로테이션
function createBalancedRotationSchedule(players, courtCount, rounds) {
    // 플레이어를 DUPR 점수 순으로 정렬 (내부 랭킹 미사용)
    const sortedPlayers = [...players].sort((a, b) => {
        const duprA = b.dupr || 0;
        const duprB = a.dupr || 0;
        const diff = duprA - duprB;
        // DUPR 2.0~3.0 범위에 최적화: 0.15 이내 차이는 같은 점수 범위로 간주
    ...
    const schedule = [];
    const totalPlayers = sortedPlayers.length;
    const totalMatches = rounds * courtCount;
    
    // 그룹 수 결정: 항상 2그룹으로 고정
    let groupCount = 2;
    
    // 그룹으로 나누기
    const groupSize = Math.floor(totalPlayers / groupCount);
    const groups = [];
    for (let g = 0; g < groupCount; g++) {
        const startIdx = g * groupSize;
        const endIdx = (g === groupCount - 1) ? totalPlayers : (g + 1) * groupSize;
        groups.push(sortedPlayers.slice(startIdx, endIdx));
    }
    ...
    for (let r = 1; r <= rounds; r++) {
        ...
        for (let c = 1; c <= courtCount; c++) {
            ...
            if (groupCount === 2) {
                // 2그룹 방식: 상위 그룹과 하위 그룹에서 각각 2명씩 선택
                const upperGroup = groups[0];
                const lowerGroup = groups[1];
                
                // 로테이션 인덱스 계산 (라운드와 코트에 따라 변화)
                const rotationBase = ((r - 1) * courtCount + (c - 1)) * 2;
                ...
                for (let attempt = 0; attempt < maxAttempts && !found; attempt++) {
                    ...
                    // 다양성을 높이기 위해 랜덤 요소 추가
                    let upperIndices = [];
                    let upperCandidates = upperGroup.filter((p, idx) => !assignedPlayersInRound.has(p.userId));
                    if (upperCandidates.length >= 2) {
                        // 랜덤 요소 추가하여 다양한 조합 시도
                        const randomOffset = Math.floor(Math.random() * Math.min(3, upperCandidates.length));
                        const baseIdx = (rotationBase + attempt + randomOffset) % upperCandidates.length;
                        ...
                        if (originalIdx1 !== -1 && originalIdx2 !== -1 && originalIdx1 !== originalIdx2) {
                            ...
                            // 1명만 찾았으면 다른 인덱스 찾기 (랜덤하게)
                            const shuffled = [...upperCandidates].sort(() => Math.random() - 0.5);
                            for (let i = 0; i < shuffled.length && upperIndices.length < 2; i++) {
                                const origIdx = upperGroup.indexOf(shuffled[i]);
                    ...
                    if (lowerCandidates.length >= 2) {
                        // 랜덤 요소 추가하여 다양한 조합 시도
                        const randomOffset = Math.floor(Math.random() * Math.min(3, lowerCandidates.length));
                        const baseIdx = (rotationBase + attempt + randomOffset) % lowerCandidates.length;
                        ...
                        if (originalIdx1 !== -1 && originalIdx2 !== -1 && originalIdx1 !== originalIdx2) {
                            ...
                            // 1명만 찾았으면 다른 인덱스 찾기 (랜덤하게)
                            const shuffled = [...lowerCandidates].sort(() => Math.random() - 0.5);
                            for (let i = 0; i < shuffled.length && lowerIndices.length < 2; i++) {
                                const origIdx = lowerGroup.indexOf(shuffled[i]);
                    ...
                    if (selectedPlayers.length === 4) {
                        ...
                        // 같은 팀원 반복 체크 (완화: 3경기 이후부터는 최근 2경기만 체크하여 다양성 확보)
                        let hasRepeatedTeammate = false;
                        const teamAIds = teamA.map(p => p.userId);
                        const teamBIds = teamB.map(p => p.userId);
                        
                        // 경기 수에 따라 체크 강도 조절 (3경기 이후부터 완화)
                        // 3경기 이후부터는 같은 팀원과 최근 2경기 내에서만 만났는지 체크
                        const recentMatchThreshold = matchNumber < 3 ? 1 : 2;
                            ...
                            if (history) {
                                for (const teammateId of teamAIds) {
                                    if (teammateId !== playerId && history.has(teammateId)) {
                                        // 3경기 이후부터는 완화: 최근 2경기만 체크
                                        // 실제로는 전체 이력이 Set이므로, 경기 수가 많을 때는 허용
                                        if (matchNumber < 3) {
                                            hasRepeatedTeammate = true;
                                            break;
                                        } else {
                                            // 3경기 이후부터는 같은 팀원과 연속으로 만나는 것만 방지
                                            // (실제로는 전체 이력이므로, 조합 중복 체크로 대체)
                                        }
                                    }
                                }
                                if (hasRepeatedTeammate && matchNumber < 3) break;
                            }
                        }
                        
                        // Team B 내에서 같은 팀원 반복 체크
                        if (!hasRepeatedTeammate || matchNumber >= 3) {
                        ...
                        // 조합 중복 체크도 완화 (3경기 이후부터는 조합 중복 허용)
                        const allowCombinationRepeat = matchNumber >= 3;
                        const isCombinationUsed = usedCombinations.has(combinationKey1) || usedCombinations.has(combinationKey2);
                        
                        if ((!isCombinationUsed || allowCombinationRepeat) && 
                            (!hasRepeatedTeammate || matchNumber >= 3)) {
                            selectedTeamA = teamA;
                            selectedTeamB = teamB;
                            ...
                            for (const playerId of teamA2Ids) {
                                ...
                                if (history) {
                                    ...
                                    if (hasRepeatedTeammate2 && matchNumber < 3) break;
                                }
                            }
                            
                            // Team B2 내에서 같은 팀원 반복 체크
                            if (!hasRepeatedTeammate2 || matchNumber >= 3) {
                            ...
                            // 조합 중복 체크도 완화
                            const isCombinationUsed2 = usedCombinations.has(combinationKey3) || usedCombinations.has(combinationKey4);
                            
                            if ((!isCombinationUsed2 || matchNumber >= 3) && 
                                (!hasRepeatedTeammate2 || matchNumber >= 3)) {
                                selectedTeamA = teamA2;
                                selectedTeamB = teamB2;
                                if (!isCombinationUsed2) {
                ...
                // 여전히 찾지 못했으면 밸런스 조합 사용 (중복 허용)
                if (!found && selectedPlayers.length === 4) {
                    const sorted = [...selectedPlayers].sort((a, b) => (b.dupr || 0) - (a.dupr || 0));
                ...
                for (let attempt = 0; attempt < maxAttempts && !found; attempt++) {
                    ...
                    for (let g = 0; g < groupCount; g++) {
                        ...
                        if (availablePlayers.length > 0) {
                            // 랜덤 요소 추가하여 다양한 조합 시도
                            const randomOffset = Math.floor(Math.random() * Math.min(3, availablePlayers.length));
                            const idx = (rotationBase + g + c + attempt + randomOffset) % availablePlayers.length;
                    ...
                    while (selectedFromGroups.length < 4 && subAttempts < 50) {
                        ...
                        for (let g = 0; g < groupCount && selectedFromGroups.length < 4; g++) {
                            ...
                            if (availablePlayers.length > 0) {
                                // 랜덤 요소 추가
                                const randomOffset = Math.floor(Math.random() * Math.min(3, availablePlayers.length));
                                const idx = (rotationBase + selectedFromGroups.length + attempt + subAttempts + randomOffset) % availablePlayers.length;
                                selectedFromGroups.push(availablePlayers[idx]);
                            }
                        }
    ...
    for (const round in matchesByRound) {
        ...
        // 각 경기의 플레이어들이 사용한 코트 점수 계산
        const matchCourtScores = roundMatches.map(match => {
            const allPlayers = [...match.teamA, ...match.teamB];
            let totalCourtUsage = 0;
            const courtUsageCount = {};
            
            allPlayers.forEach(player => {
                if (!playerCourtHistory.has(player.userId)) {
                    playerCourtHistory.set(player.userId, new Map());
                }
                const history = playerCourtHistory.get(player.userId);
                
                // 각 코트별 사용 횟수 계산
                for (let c = 1; c <= courtCount; c++) {
                    if (!courtUsageCount[c]) courtUsageCount[c] = 0;
                    courtUsageCount[c] += (history.get(c) || 0);
                }
            });
            
            return { match, courtUsageCount };
        });
        
        // 코트 배정: 각 경기를 가장 적게 사용한 코트에 배정
        const assignedCourts = new Set();
        const sortedMatches = [...matchCourtScores].sort((a, b) => {
            // 총 사용 횟수가 적은 경기부터 배정
            const totalA = Object.values(a.courtUsageCount).reduce((sum, count) => sum + count, 0);
            const totalB = Object.values(b.courtUsageCount).reduce((sum, count) => sum + count, 0);
            return totalA - totalB;
        });
        
        sortedMatches.forEach(({ match, courtUsageCount }) => {
            // 사용 횟수가 가장 적은 코트 찾기
            let bestCourt = 1;
            let minUsage = courtUsageCount[1] || 0;
            
            for (let c = 1; c <= courtCount; c++) {
                const usage = courtUsageCount[c] || 0;
                if (usage < minUsage || (usage === minUsage && !assignedCourts.has(c))) {
                    minUsage = usage;
                    bestCourt = c;
                }
            }
            
            // 이미 배정된 코트가 있으면 다음으로 적게 사용한 코트 선택
            if (assignedCourts.has(bestCourt)) {
                for (let c = 1; c <= courtCount; c++) {
                    if (!assignedCourts.has(c)) {
                        bestCourt = c;
                        break;
                    }
                }
            ...
            allPlayers.forEach(player => {
                ...
                const history = playerCourtHistory.get(player.userId);
                history.set(bestCourt, (history.get(bestCourt) || 0) + 1);
            });
...
/**
 * 피클볼 밸런스 모드 매칭 엔진 (V2)
 * 요구사항에 맞는 8경기 고정 시나리오 구현
 */
function buildPickleballBalancedSchedule(players, courtCount) {
    // 코트 수 계산: max(1, floor(players.length / 4))
    const calculatedCourtCount = Math.max(1, Math.floor(players.length / 4));
    const finalCourtCount = Math.min(calculatedCourtCount, courtCount);
    ...
    players.forEach(p => {
        playerState[p.userId] = {
            gamesPlayed: 0,
            teammateCount: {}, // { "id1-id2": count }
            lastGameIndex: 0
        };
    });
    
    // CourtMatch 타입: { gameIndex: number, courtIndex: number, teamA: Player[], teamB: Player[] }
    const schedule = [];
    
    // 후보군 선택 함수: gamesPlayed <= minG + 1 조건 충족
    function getPriorityCandidates(allPlayers) {
        if (allPlayers.length === 0) return [];
        const minG = Math.min(...allPlayers.map(p => playerState[p.userId]?.gamesPlayed || 0));
        return allPlayers.filter(p => 
            (playerState[p.userId]?.gamesPlayed || 0) <= minG + 1
        );
    }
    
    // 팀 분할 점수 계산 함수
    function calculateTeamScore(teamA, teamB, w1 = 1, w2 = 100) {
        // duprDiff: 두 팀의 총 DUPR 차이 절대값
        const duprA = teamA.reduce((sum, p) => sum + (p.dupr || 0), 0);
        const duprB = teamB.reduce((sum, p) => sum + (p.dupr || 0), 0);
        const duprDiff = Math.abs(duprA - duprB);
        
        // teammatePenalty: 기존 teammateCount 합
        let teammatePenalty = 0;
        if (teamA.length >= 2 && teamB.length >= 2) {
            const pair1 = [teamA[0].userId, teamA[1].userId].sort().join('-');
            const pair2 = [teamB[0].userId, teamB[1].userId].sort().join('-');
            
            teammatePenalty += (playerState[teamA[0].userId]?.teammateCount?.[pair1] || 0);
            teammatePenalty += (playerState[teamB[0].userId]?.teammateCount?.[pair2] || 0);
        }
        
        return w1 * duprDiff + w2 * teammatePenalty;
    }
    
    // 4명에서 가능한 3가지 팀 분할 평가
    function evaluateTeamSplits(fourPlayers) {
        const splits = [
            { teamA: [fourPlayers[0], fourPlayers[3]], teamB: [fourPlayers[1], fourPlayers[2]] },
            { teamA: [fourPlayers[0], fourPlayers[2]], teamB: [fourPlayers[1], fourPlayers[3]] },
            { teamA: [fourPlayers[0], fourPlayers[1]], teamB: [fourPlayers[2], fourPlayers[3]] }
        ];
        ...
        for (const split of splits) {
            const score = calculateTeamScore(split.teamA, split.teamB);
            if (score < bestScore) {
                bestScore = score;
                bestSplit = split;
            }
    ...
    // teammateCount 업데이트
    function updateTeammateCount(teamA, teamB) {
        // TeamA 내부
        if (teamA.length >= 2) {
            const pairA = [teamA[0].userId, teamA[1].userId].sort().join('-');
            if (playerState[teamA[0].userId]) {
                playerState[teamA[0].userId].teammateCount[pairA] = 
                    (playerState[teamA[0].userId].teammateCount[pairA] || 0) + 1;
            }
            if (playerState[teamA[1].userId]) {
                playerState[teamA[1].userId].teammateCount[pairA] = 
                    (playerState[teamA[1].userId].teammateCount[pairA] || 0) + 1;
            }
        }
        ...
        if (teamB.length >= 2) {
            ...
            if (playerState[teamB[0].userId]) {
                playerState[teamB[0].userId].teammateCount[pairB] = 
                    (playerState[teamB[0].userId].teammateCount[pairB] || 0) + 1;
            }
            if (playerState[teamB[1].userId]) {
                playerState[teamB[1].userId].teammateCount[pairB] = 
                    (playerState[teamB[1].userId].teammateCount[pairB] || 0) + 1;
            }
        }
    }
    
    // gamesPlayed 업데이트
    function updateGamesPlayed(players) {
        players.forEach(p => {
            if (playerState[p.userId]) {
                playerState[p.userId].gamesPlayed++;
            }
        });
    }
    
    // 1,2경기: 상위 실력 집중 모드
    function createEliteMode(gameIndex, candidates) {
        // DUPR 내림차순 정렬
        const sorted = [...candidates].sort((a, b) => (b.dupr || 0) - (a.dupr || 0));
        const elite = sorted.slice(0, 4);
        
        if (elite.length < 4) {
            return null; // 인원 부족 시 null 반환
        }
        
        let teamA, teamB;
        if (gameIndex === 1) {
            // 1경기: [최강, 차차차강] vs [차강, 차차강]
            teamA = [elite[0], elite[3]];
            teamB = [elite[1], elite[2]];
        } else if (gameIndex === 2) {
            // 2경기: [최강, 차차강] vs [차강, 차차차강]
            teamA = [elite[0], elite[2]];
            teamB = [elite[1], elite[3]];
        } else {
            return null;
        }
        
        return { teamA, teamB, usedPlayers: elite };
    }
    
    // 3,4,7,8경기: 밸런스 모드
    function createBalanceMode(candidates) {
        if (candidates.length < 4) {
            return null;
        }
        
        // 후보군에서 4명 조합 생성 (여러 개 시도)
        const priorityCandidates = getPriorityCandidates(candidates);
        const availableCandidates = priorityCandidates.length >= 4 ? priorityCandidates : candidates;
        
        // DUPR 순으로 정렬하여 밸런스 조합 생성
        const sorted = [...availableCandidates].sort((a, b) => (b.dupr || 0) - (a.dupr || 0));
        ...
        // 최대 10개 조합 시도
        for (let start = 0; start <= Math.min(5, sorted.length - 4); start++) {
            const fourPlayers = sorted.slice(start, start + 4);
            if (fourPlayers.length < 4) break;
            
            const split = evaluateTeamSplits(fourPlayers);
            if (split) {
                const score = calculateTeamScore(split.teamA, split.teamB);
                if (score < bestScore) {
                    bestScore = score;
                    bestCombination = { ...split, usedPlayers: fourPlayers };
                }
            }
        }
    ...
    // 5,6경기: 강·약 믹스 모드
    function createMixMode(gameIndex, courtIndex, candidates) {
        if (candidates.length < 4) {
            return null;
        }
        ...
        // high = 상위 2*코트수, low = 하위 2*코트수
        const highCount = Math.min(2 * finalCourtCount, Math.floor(sorted.length / 2));
        const lowCount = Math.min(2 * finalCourtCount, sorted.length - highCount);
        
        if (highCount < 2 || lowCount < 2) {
            // 강·약 구분 불가능하면 밸런스 모드로 fallback
            return createBalanceMode(candidates);
        }
        
        const high = sorted.slice(0, highCount);
        const low = sorted.slice(sorted.length - lowCount);
        
        // gamesPlayed 적은 사람 우선 순위 반영
        high.sort((a, b) => {
            const gamesA = playerState[a.userId].gamesPlayed;
            const gamesB = playerState[b.userId].gamesPlayed;
        ...
        low.sort((a, b) => {
            ...
            const gamesB = playerState[b.userId].gamesPlayed;
            if (gamesA !== gamesB) return gamesA - gamesB;
            return (a.dupr || 0) - (b.dupr || 0); // 낮은 순
        });
        
        // 코트별로 배정 (courtIndex에 따라 다른 플레이어 선택)
        if (high.length >= 2 * courtIndex && low.length >= 2 * courtIndex) {
            let teamA, teamB;
            const k = courtIndex - 1; // 0-based index
            
            if (gameIndex === 5) {
                // 패턴 A: 최강·최약 / 차강·차약
                // 코트 k: 팀A = [high(2k), low(2k+1)], 팀B = [high(2k+1), low(2k)]
                teamA = [high[2 * k], low[2 * k + 1]];
                teamB = [high[2 * k + 1], low[2 * k]];
            } else if (gameIndex === 6) {
                // 패턴 B: 최강·차약 / 차강·최약
                // 코트 k: 팀A = [high(2k), low(2k)], 팀B = [high(2k+1), low(2k+1)]
                teamA = [high[2 * k], low[2 * k]];
                teamB = [high[2 * k + 1], low[2 * k + 1]];
            } else {
                return null;
            }
            
            return { teamA, teamB, usedPlayers: [...teamA, ...teamB] };
        }
        
        // 인원이 부족하면 밸런스 모드로 fallback
        return createBalanceMode(candidates);
    }
    
    // 8경기 생성
    const allPlayers = players; // players는 이미 { userId, userName, dupr } 형식의 객체 배열
    
    for (let gameIndex = 1; gameIndex <= 8; gameIndex++) {
        // 각 코트별로 경기 생성
        for (let courtIndex = 1; courtIndex <= finalCourtCount; courtIndex++) {
            // 이미 이 경기에서 배정된 플레이어 제외
            const usedInThisGame = new Set();
            schedule.filter(m => m.gameIndex === gameIndex).forEach(m => {
                m.teamA.forEach(p => usedInThisGame.add(p.userId));
                m.teamB.forEach(p => usedInThisGame.add(p.userId));
            });
            
            const candidates = allPlayers.filter(p => !usedInThisGame.has(p.userId));
            ...
            if (gameIndex === 1 || gameIndex === 2) {
                ...
                if (courtIndex === 1) {
                    // 첫 번째 코트만 상위 집중 모드
                    match = createEliteMode(gameIndex, candidates);
                } else {
                    // 나머지 코트는 밸런스 모드
                    match = createBalanceMode(candidates);
                }
            } else if (gameIndex === 3 || gameIndex === 4 || gameIndex === 7 || gameIndex === 8) {
                // 밸런스 모드
                match = createBalanceMode(candidates);
            } else if (gameIndex === 5 || gameIndex === 6) {
                // 강·약 믹스 모드
                match = createMixMode(gameIndex, courtIndex, candidates);
            }
            
            if (match && match.teamA && match.teamB) {
                // teammateCount 업데이트
                updateTeammateCount(match.teamA, match.teamB);
                
                // gamesPlayed 업데이트
                updateGamesPlayed([...match.teamA, ...match.teamB]);
    ...
    // 기존 형식으로 변환 (round, court, roundNumber, teamA, teamB)
    const convertedSchedule = schedule.map(m => ({
        round: m.gameIndex,
        court: m.courtIndex,
        roundNumber: m.gameIndex,
        teamA: m.teamA,
        teamB: m.teamB
    }));
    ...
    // 배정되지 않은 플레이어 찾기
    const assignedPlayerIds = new Set();
    convertedSchedule.forEach(m => {
        m.teamA.forEach(p => assignedPlayerIds.add(p.userId));
        m.teamB.forEach(p => assignedPlayerIds.add(p.userId));
    });
    
    const unassignedPlayers = players.filter(p => !assignedPlayerIds.has(p.userId));
    
    return { schedule: convertedSchedule, unassignedPlayers };
}
...
function buildMatchSchedule(players, courtCount, rounds, playerCourtMap = {}, teamMode = 'random') {
    ...
    if (teamMode === 'grouped') {
        ...
        // 코트별로 그룹화하여 배정 (잘하는 사람을 1코트에 몰아서 배정)
        // 예: 15명, 3코트 -> 코트1: 상위 5명, 코트2: 중위 5명, 코트3: 하위 5명
        const playersPerCourt = Math.floor(sortedPlayers.length / courtCount);
        const remainder = sortedPlayers.length % courtCount;
        
        let playerIndex = 0;
        for (let c = 1; c <= courtCount; c++) {
            // 각 코트에 배정할 인원 수 계산 (나머지는 앞쪽 코트에 배정)
            const courtSize = playersPerCourt + (c <= remainder ? 1 : 0);
            ...
            // 잘하는 사람부터 순서대로 해당 코트에 배정
            for (let i = 0; i < courtSize && playerIndex < sortedPlayers.length; i++) {
                courtPlayers[c].push(sortedPlayers[playerIndex]);
                playerIndex++;
            }
        }
    } else {
        ...
        // 코트 수는 인원수에 따라 결정되지만 (4명=1코트, 8명=2코트, 12명 이상=3코트),
        // 플레이어는 각 경기마다 동적으로 코트에 배정됨
    }
    ...
    if (teamMode === 'random' || teamMode === 'balanced') {
        ...
        // 밸런스 모드: 코트별 경기 번호 추적 및 상위 4명 분리
        const courtMatchNumbers = {}; // 코트별 경기 번호: court -> matchNumber
        ...
        const totalMatches = rounds * courtCount; // 총 경기 수는 rounds * courtCount
        ...
        if (teamMode === 'balanced') {
            ...
            // 코트 2개: 최강, 차강, 차차강, 차차차강, 차차차약, 차차약, 차약, 최약 (8명)
            // 코트 3개: 상위 6명 + 하위 6명 (12명)
                    ...
                    if (teamMode === 'balanced') {
                        ...
                        if (topFourPlayers && topFourPlayers.length > 0) {
                                ...
                                if (matchNum === 1 || matchNum === 2) {
                                    ...
                                    console.log(`🔍 ${matchNum}경기 플레이어 선택 - 라운드 ${r}, 코트 ${c}:`);
                                    ...
                                    if (courtCount >= 3 && allSortedFor12.length >= 12) {
                                        ...
                                        if (allSortedFor12.length >= 4) {
                                            ...
                                            availablePlayers = allSortedFor12;
                                        }
                                        ...
                                        if (allSortedFor12.length >= 8) {
                                            ...
                                            // 8명 미만이면 하위 4명 선택
                                            availablePlayers = allSortedFor12.slice(-4);
                                        } else {
                                            availablePlayers = allSortedFor12;
                                        }
                                        console.log(`  - 짝수 코트(하위): ${allSortedFor12.length >= 8 ? '5-8번' : '하위 4명'} 선택`);
                                    }
                                    console.log(`  - 선택된 플레이어: ${availablePlayers.map((p, idx) => `${idx+1}번:${p.userName}(${p.dupr || 0})`).join(', ')}`);
                                } else if (matchNum === 3 || matchNum === 4) {
                                    // 3,4 경기: 전체 풀에서 조화롭게 선택 (참여 횟수 고려 + DUPR 밸런스)
                                    // 전체 플레이어를 참여 횟수와 DUPR 점수를 고려하여 선택
                                    const allAvailable = [...allAvailablePlayers].filter(p => 
                                        !assignedPlayersInRound.has(p.userId)
                                    );
                                    
                                    // 참여 횟수 기준으로 정렬 (적은 순)
                                    const sortedByCount = [...allAvailable].sort((a, b) => {
                                    ...
                                    // 참여 횟수가 가장 적은 사람들 중에서 전체 풀의 밸런스를 고려하여 선택
                                    const minCount = Math.min(...sortedByCount.map(p => playerPlayCount[p.userId] || 0));
                                    const candidatesWithMinCount = sortedByCount.filter(p => (playerPlayCount[p.userId] || 0) === minCount);
                                    
                                    if (candidatesWithMinCount.length >= 4) {
                                        // 참여 횟수가 가장 적은 사람이 4명 이상이면, 전체 풀의 밸런스를 고려하여 선택
                                        // 최강, 차강, 차약, 최약 조화롭게 선택
                                        if (candidatesWithMinCount.length >= 8) {
                                            // 8명 이상이면 상위 2명, 중위 2명 선택
                                            availablePlayers = [
                                                candidatesWithMinCount[0], // 최강
                                                candidatesWithMinCount[Math.floor(candidatesWithMinCount.length / 2) - 1], // 중위 상
                                                candidatesWithMinCount[Math.floor(candidatesWithMinCount.length / 2)], // 중위 하
                                                candidatesWithMinCount[candidatesWithMinCount.length - 1] // 최약
                                            ];
                                        } else {
                                            // 4-7명이면 균등하게 분산하여 선택
                                            const step = Math.floor(candidatesWithMinCount.length / 4);
                                            availablePlayers = [
                                                candidatesWithMinCount[0], // 최강
                                                candidatesWithMinCount[step], // 상위 중
                                                candidatesWithMinCount[candidatesWithMinCount.length - step - 1], // 하위 중
                                                candidatesWithMinCount[candidatesWithMinCount.length - 1] // 최약
                                            ];
                                        }
                                    } else {
                                        // 참여 횟수가 가장 적은 사람이 4명 미만이면 추가로 선택
                                        const needed = 4 - candidatesWithMinCount.length;
                                        const nextMinCount = sortedByCount.find(p => (playerPlayCount[p.userId] || 0) > minCount);
                                        if (nextMinCount) {
                                            const nextMinCountValue = playerPlayCount[nextMinCount.userId] || 0;
                                            const candidatesWithNextMinCount = sortedByCount.filter(p => 
                                                (playerPlayCount[p.userId] || 0) === nextMinCountValue
                                            );
                                            const combined = [...candidatesWithMinCount, ...candidatesWithNextMinCount];
                                            // 전체 풀의 밸런스를 고려하여 선택
                                            if (combined.length >= 4) {
                                                availablePlayers = [
                                                    combined[0], // 최강
                                                    combined[Math.floor(combined.length / 2) - 1], // 중위 상
                                                    combined[Math.floor(combined.length / 2)], // 중위 하
                                                    combined[combined.length - 1] // 최약
                                                ].slice(0, 4);
                                            } else {
                                                availablePlayers = combined.slice(0, 4);
                                            }
                                        } else {
                                            // 그래도 부족하면 전체에서 선택
                                            availablePlayers = sortedByCount.slice(0, 4);
                                        }
                                    }
                                    
                                    console.log(`🎯 ${matchNum}경기 플레이어 선택 - 라운드 ${r}, 코트 ${c}: 전체 풀에서 조화롭게 선택`);
                                    console.log(`  - 선택된 플레이어: ${availablePlayers.map(p => `${p.userName}(${p.dupr || 0})`).join(', ')}`);
                                } else if (matchNum === 5 || matchNum === 6) {
                                    // 5,6 경기: 전체 풀에서 플레이어 선택 (간단한 인덱스 조합)
                                    // 전체 플레이어를 DUPR 점수 순으로 정렬 (1번=최강, 2번=차강, ..., N번=최약)
                                    ...
                                    console.log(`  - 선택된 플레이어: ${availablePlayers.map((p, idx) => `${idx+1}번:${p.userName}(${p.dupr || 0})`).join(', ')}`);
                                } else if (matchNum === 7 || matchNum === 8) {
                                    // 7,8 경기: 전체 풀에서 조화롭게 선택 (참여 횟수 고려 + DUPR 밸런스)
                                    // 전체 플레이어를 참여 횟수와 DUPR 점수를 고려하여 선택
                                    const allAvailable = [...allAvailablePlayers].filter(p => 
                                        !assignedPlayersInRound.has(p.userId)
                                    );
                                    ...
                                    const allSortedFor78 = [...allAvailable].sort((a, b) => {
                                        const duprA = b.dupr || 0;
                                        const duprB = a.dupr || 0;
                                        const diff = duprA - duprB;
                                        if (Math.abs(diff) < 0.0001) {
                                    ...
                                    if (needed > 0 && notPlayed.length > 0) {
                                        availablePlayers = [...notPlayed, ...topFourByCount.slice(0, needed)];
                                    } else if (notPlayed.length >= 4) {
                                        availablePlayers = notPlayed.slice(0, 4);
                                    } else {
                        ...
                        // 디버깅: 밸런스 모드에서 플레이어 선택 상태 확인
                        if (teamMode === 'balanced') {
                            console.log(`🔍 밸런스 모드 디버깅 - 라운드 ${r}, 코트 ${c}, 경기 ${targetMatchNum}:`);
                            console.log(`  - 전체 플레이어: ${shuffledAllPlayers.length}명`);
                            console.log(`  - 같은 라운드 배정된 플레이어: ${assignedPlayersInRound.size}명`);
                            console.log(`  - 사용 가능한 플레이어: ${availableThisMatch.length}명`);
                            if (topFourPlayers) {
                                console.log(`  - 상위 4명: ${topFourPlayers.map(p => p.userName).join(', ')}`);
                            }
                            if (availableThisMatch.length > 0) {
                                console.log(`  - 선택된 플레이어: ${availableThisMatch.map(p => p.userName).join(', ')}`);
                            }
                        }
                        ...
                        if (teamMode === 'balanced') {
                            // 밸런스 모드: 이미 선택된 플레이어 사용 (경기 번호에 따라 이미 선택됨)
                            fourPlayers = availableThisMatch.slice(0, 4);
                        } else {
                            // 랜덤 모드: 참여 횟수가 적은 플레이어들을 우선 선택
                        ...
                        if (!fourPlayers || fourPlayers.length < 4) {
                            console.warn(`⚠️ 코트 ${c}, 경기 ${r} 생성 실패: 플레이어 부족 (${fourPlayers ? fourPlayers.length : 0}명)`);
                            continue;
                        }
                        ...
                        if (teamMode === 'balanced') {
                            ...
                            if (matchNum === 1 || matchNum === 2) {
                                // 1,2 경기: 간단한 고정 인덱스 조합
                                // 플레이어를 DUPR 점수 순으로 정렬 (sorted[0]=1번, sorted[1]=2번, ..., sorted[3]=4번)
                                // 코트별로 4명 선택 후 인덱스로 팀 구성
                                ...
                                console.log(`  - sorted: ${sorted.map((p, idx) => `${idx+1}번:${p.userName}(${p.dupr || 0})`).join(', ')}`);
                                console.log(`  - Team A: ${teamAIds.map(id => sorted.find(s => s.userId === id)?.userName).join(', ')}`);
                                console.log(`  - Team B: ${teamBIds.map(id => sorted.find(s => s.userId === id)?.userName).join(', ')}`);
                                
                                // 1,2 경기는 고정 조합이므로 바로 추가
                                teamConfigs.push({ 
                                    teamA: teamAIds, 
                                    teamB: teamBIds 
                                });
                                ...
                                for (const prev of previousMatchConfigs) {
                                    ...
                                    if (prev.matchNum === 3 || prev.matchNum === 4 || prev.matchNum === 1 || prev.matchNum === 2) {
                                ...
                                // 전체 풀에서 조화롭게 선택된 4명을 DUPR 점수 순으로 정렬
                                const sorted = [...fourPlayers].sort((a, b) => {
                                    const duprA = b.dupr || 0;
                                    const duprB = a.dupr || 0;
                                    const diff = duprA - duprB;
                                ...
                                // 가능한 조합들 시도 (최강+차강 vs 차약+최약 제외)
                                const possibleCombos = [
                                    [[sorted[0], sorted[3]], [sorted[1], sorted[2]]], // 최강+최약 vs 차강+차약
                                    [[sorted[0], sorted[2]], [sorted[1], sorted[3]]]  // 최강+차약 vs 차강+최약
                                ];
                                
                                console.log(`🔍 ${matchNum}경기 팀 조합 생성 - 라운드 ${r}, 코트 ${c}:`);
                                console.log(`  - 선택된 플레이어: ${fourPlayers.map(p => `${p.userName}(${p.dupr || 0})`).join(', ')}`);
                                ...
                                for (const combo of possibleCombos) {
                                    ...
                                    if (forbiddenCombinations.has(comboKey)) {
                                        console.log(`  - 조합 ${comboKey}는 금지된 팀 조합입니다.`);
                                        continue;
                                    }
                                    
                                    // 4경기일 때: 3경기와 같은 팀원 조합인지 확인 (코트 무관, 전체 체크)
                                    let hasSameTeammateCombo = false;
                                    if (matchNum === 4) {
                                        console.log(`  - 4경기: 3경기 팀원 조합 체크 시작 (코트 ${c})`);
                                        for (const prev of previousMatchConfigs) {
                                            if (prev.matchNum === 3) {
                                                ...
                                                console.log(`    - 3경기 코트${prev.court || '?'}: TeamA=[${prevTeamA.join(',')}], TeamB=[${prevTeamB.join(',')}]`);
                                                console.log(`    - 4경기 코트${c} 후보: TeamA=[${currentTeamA.join(',')}], TeamB=[${currentTeamB.join(',')}]`);
                                                ...
                                                if (prevTeamASet.size === currentTeamASet.size && 
                                                    ...
                                                    console.log(`    ❌ 3경기 TeamA와 4경기 TeamA가 같은 팀원 조합입니다!`);
                                                    break;
                                                }
                                                ...
                                                if (prevTeamASet.size === currentTeamBSet.size && 
                                                    ...
                                                    hasSameTeammateCombo = true;
                                                    console.log(`    ❌ 3경기 TeamA와 4경기 TeamB가 같은 팀원 조합입니다!`);
                                                    break;
                                                }
                                                ...
                                                if (prevTeamBSet.size === currentTeamASet.size && 
                                                    ...
                                                    hasSameTeammateCombo = true;
                                                    console.log(`    ❌ 3경기 TeamB와 4경기 TeamA가 같은 팀원 조합입니다!`);
                                                    break;
                                                }
                                                ...
                                                if (prevTeamBSet.size === currentTeamBSet.size && 
                                                    ...
                                                    hasSameTeammateCombo = true;
                                                    console.log(`    ❌ 3경기 TeamB와 4경기 TeamB가 같은 팀원 조합입니다!`);
                                                    break;
                                                }
                                            }
                                        }
                                        if (!hasSameTeammateCombo) {
                                            console.log(`    ✅ 3경기와 다른 팀원 조합입니다.`);
                                        }
                                    }
                                    ...
                                    if (!hasSameTeammateCombo) {
                                        ...
                                        console.log(`  ✅ 유효한 조합 추가: TeamA=[${teamA}], TeamB=[${teamB}]`);
                                        break; // 첫 번째 유효한 조합 사용
                                    } else {
                                        console.log(`  ⚠️ 같은 팀원 조합으로 인해 건너뜀: TeamA=[${teamA}], TeamB=[${teamB}]`);
                                    }
                                }
                                
                                // 유효한 조합을 찾지 못했으면 첫 번째 조합 사용 (fallback)
                                if (teamConfigs.length === 0) {
                                    console.log(`  ⚠️ 유효한 조합을 찾지 못해 fallback 사용`);
                                    teamConfigs.push(
                        ...
                        if (teamMode === 'balanced' && (targetMatchNum === 5 || targetMatchNum === 6)) {
                            if (fourPlayers && fourPlayers.length === 4) {
                                ...
                                // sorted[0]=1번, sorted[1]=2번, sorted[2]=3번, sorted[3]=4번 (선택된 4명 기준)
                                let selectedTeamA, selectedTeamB;
                                if (targetMatchNum === 5) {
                                    // 5경기: 코트1 = 1,8 vs 2,7, 코트2 = 3,6 vs 4,5
                                    // fourPlayers가 이미 코트별로 선택되었으므로, 각 코트에서 1,4 vs 2,3 조합 사용
                                    selectedTeamA = [sorted[0], sorted[3]]; // 1번, 4번
                                    selectedTeamB = [sorted[1], sorted[2]]; // 2번, 3번
                                } else { // targetMatchNum === 6
                                    // 6경기: 코트1 = 1,7 vs 2,8, 코트2 = 3,5 vs 4,6
                                    // fourPlayers가 이미 코트별로 선택되었으므로, 각 코트에서 1,3 vs 2,4 조합 사용
                                    selectedTeamA = [sorted[0], sorted[2]]; // 1번, 3번
                                    selectedTeamB = [sorted[1], sorted[3]]; // 2번, 4번
                                }
                                
                                console.log(`🔍 ${targetMatchNum}경기 팀 구성 - 라운드 ${r}, 코트 ${c}:`);
                                console.log(`  - sorted: ${sorted.map((p, idx) => `${idx+1}번:${p.userName}(${p.dupr || 0})`).join(', ')}`);
                                console.log(`  - Team A: ${selectedTeamA.map(p => p.userName).join(', ')}`);
                                console.log(`  - Team B: ${selectedTeamB.map(p => p.userName).join(', ')}`);
                                ...
                                if (teamMode === 'balanced') {
                                    if (targetMatchNum === 5 || targetMatchNum === 6 || targetMatchNum === 1 || targetMatchNum === 2 || targetMatchNum === 7 || targetMatchNum === 8) {
                                        fourPlayers.forEach(player => {
                                ...
                                console.log(`✅ 코트 ${c}, 경기 ${r} 생성 완료 (${targetMatchNum}경기): ${selectedTeamA.map(p => p.userName).join(',')} vs ${selectedTeamB.map(p => p.userName).join(',')}`);
                        ...
                        for (const config of teamConfigs) {
                            ...
                            if (targetMatchNum === 1 || targetMatchNum === 2) {
                                ...
                                // 이전 경기 조합에 추가 (경기 번호 및 코트 타입 포함)
                                const courtType = c % 2 === 1 ? 'upper' : 'lower';
                                const teamAKeySorted = teamAKey.split(',').sort().join(',');
                                const teamBKeySorted = teamBKey.split(',').sort().join(',');
                                previousMatchConfigs.push({ 
                                    teamAIds: teamAKeySorted, 
                                    teamBIds: teamBKeySorted,
                                    matchNum: targetMatchNum,
                                    courtType: courtType
                                });
                                console.log(`  📝 previousMatchConfigs에 추가: 경기 ${targetMatchNum}, 코트 ${c} (${courtType}), Team A [${teamAKeySorted}], Team B [${teamBKeySorted}]`);
                                ...
                                found = true;
                                break; // 1,2 경기는 첫 번째 조합만 사용
                            }
                            
                            // 1,2 경기가 아닌 경우 기존 체크 로직 사용
                            ...
                            if (targetMatchNum === 4) {
                                ...
                                if (!hasRepeatedTeammate) {
                                    ...
                                    console.log(`    ⚠️ 3경기와 같은 팀원 조합이 발견되어 이 조합은 제외됩니다.`);
                                }
                            }
                            
                            // 7경기일 때: 3,4경기에서 같은 팀이었던 조합 방지
                            if (targetMatchNum === 7) {
                                for (const prev of previousMatchConfigs) {
                                    if (prev.matchNum === 3 || prev.matchNum === 4) {
                                        ...
                                        // 3,4경기의 Team A와 7경기의 Team A가 같은 팀원 조합인지 확인
                                        const prevTeamASet = new Set(prevTeamA);
                                        const currentTeamASet = new Set(teamAIds);
                                        if (prevTeamASet.size === currentTeamASet.size && 
                                            [...prevTeamASet].every(id => currentTeamASet.has(id))) {
                                        ...
                                        // 3,4경기의 Team A와 7경기의 Team B가 같은 팀원 조합인지 확인
                                        const currentTeamBSet = new Set(teamBIds);
                                        if (prevTeamASet.size === currentTeamBSet.size && 
                                            [...prevTeamASet].every(id => currentTeamBSet.has(id))) {
                                            hasRepeatedTeammate = true;
                                        ...
                                        // 3,4경기의 Team B와 7경기의 Team A가 같은 팀원 조합인지 확인
                                        const prevTeamBSet = new Set(prevTeamB);
                                        if (prevTeamBSet.size === currentTeamASet.size && 
                                            [...prevTeamBSet].every(id => currentTeamASet.has(id))) {
                                            hasRepeatedTeammate = true;
                                        ...
                                        // 3,4경기의 Team B와 7경기의 Team B가 같은 팀원 조합인지 확인
                                        if (prevTeamBSet.size === currentTeamBSet.size && 
                                            [...prevTeamBSet].every(id => currentTeamBSet.has(id))) {
                                            hasRepeatedTeammate = true;
                                            break;
                            ...
                            if (targetMatchNum === 8) {
                                for (const prev of previousMatchConfigs) {
                                    if (prev.matchNum === 3 || prev.matchNum === 4 || prev.matchNum === 7) {
                                        ...
                                        // 3,4,7경기의 Team A와 8경기의 Team A가 같은 팀원 조합인지 확인
                                        const prevTeamASet = new Set(prevTeamA);
                                        const currentTeamASet = new Set(teamAIds);
                                        if (prevTeamASet.size === currentTeamASet.size && 
                                            [...prevTeamASet].every(id => currentTeamASet.has(id))) {
                                        ...
                                        // 3,4,7경기의 Team A와 8경기의 Team B가 같은 팀원 조합인지 확인
                                        const currentTeamBSet = new Set(teamBIds);
                                        if (prevTeamASet.size === currentTeamBSet.size && 
                                            [...prevTeamASet].every(id => currentTeamBSet.has(id))) {
                                            hasRepeatedTeammate = true;
                                        ...
                                        // 3,4,7경기의 Team B와 8경기의 Team A가 같은 팀원 조합인지 확인
                                        const prevTeamBSet = new Set(prevTeamB);
                                        if (prevTeamBSet.size === currentTeamASet.size && 
                                            [...prevTeamBSet].every(id => currentTeamASet.has(id))) {
                                            hasRepeatedTeammate = true;
                                        ...
                                        // 3,4,7경기의 Team B와 8경기의 Team B가 같은 팀원 조합인지 확인
                                        if (prevTeamBSet.size === currentTeamBSet.size && 
                                            [...prevTeamBSet].every(id => currentTeamBSet.has(id))) {
                                            hasRepeatedTeammate = true;
                                            break;
                            ...
                            // 전체 이력에서 같은 팀원 반복 체크 (4경기, 7경기, 8경기가 아닌 경우에만)
                            if (!hasRepeatedTeammate && targetMatchNum !== 4 && targetMatchNum !== 7 && targetMatchNum !== 8) {
                                // Team A 내에서 같은 팀원 반복 체크
                                for (const playerId of teamAIds) {
                                    const history = teammateHistory.get(playerId);
                                    if (history) {
                        ...
                        if (!found) {
                            ...
                            console.log(`✅ 코트 ${c}, 경기 ${r} 생성 완료 (fallback): ${selectedTeamA.map(p => p.userName).join(',')} vs ${selectedTeamB.map(p => p.userName).join(',')}`);
                        }
        ...
        if (totalPlayers % 4 !== 0) {
            ...
            // 밸런스 모드: 상위 4명 분리 (4명 이상인 경우)
            let topFourPlayers = null;
            let remainingPlayers = null;
            if (teamMode === 'balanced' && totalPlayers >= 4) {
                // 상위 4명 고정 (최강, 차강, 차약, 최약)
                topFourPlayers = sortedAllPlayers.slice(0, 4);
                // 나머지 플레이어들 (4명 초과인 경우)
                remainingPlayers = totalPlayers > 4 ? sortedAllPlayers.slice(4) : [];
            }
            
            // 참여 이력 추적 (5,6 경기에서 사용)
            ...
            for (let r = 1; r <= rounds; r++) {
                ...
                if (teamMode === 'balanced' && topFourPlayers) {
                    courtMatchNumber++;
                    const matchNum = courtMatchNumber;
                    ...
                    if (matchNum === 1 || matchNum === 2) {
                        ...
                        if (c % 2 === 1) {
                            ...
                            if (remainingPlayers.length >= 4) {
                                // 하위 4명 선택 (DUPR 점수 낮은 순)
                                const sortedRemaining = [...remainingPlayers].sort((a, b) => {
                                    const duprA = a.dupr || 0;
                                    const duprB = b.dupr || 0;
                                    return duprA - duprB; // 낮은 순으로 정렬
                                });
                                availablePlayers = sortedRemaining.slice(0, 4);
                            } else {
                                // 하위 플레이어가 부족하면 상위 플레이어로 보충
                                const needed = 4 - remainingPlayers.length;
                                availablePlayers = [...remainingPlayers, ...topFourPlayers.slice(-needed)];
                            }
                        }
                    } else if (matchNum === 3 || matchNum === 4) {
                        // 3,4 경기: 5,6,1,2,7,8 경기에 참여하지 않은 사람들 우선 선택
                        // 참여 횟수가 적은 사람들을 우선 선택 (참여 안한 사람 우선)
                        const allAvailable = [...topFourPlayers, ...remainingPlayers];
                        
                        // 참여 횟수 기준으로 정렬 (적은 순 → 같은 횟수면 랜덤)
                        const sortedByCount = allAvailable.sort((a, b) => {
                            const countA = playerPlayCount[a.userId] || 0;
                            const countB = playerPlayCount[b.userId] || 0;
                            if (countA !== countB) {
                        ...
                        // 참여 횟수가 가장 적은 사람들 우선 선택 (최소 4명)
                        const minCount = Math.min(...sortedByCount.map(p => playerPlayCount[p.userId] || 0));
                        const candidatesWithMinCount = sortedByCount.filter(p => (playerPlayCount[p.userId] || 0) === minCount);
                        
                        if (candidatesWithMinCount.length >= 4) {
                            // 참여 횟수가 가장 적은 사람이 4명 이상이면 그 중에서 선택
                            availablePlayers = candidatesWithMinCount.slice(0, 4);
                        } else {
                            // 참여 횟수가 가장 적은 사람이 4명 미만이면 추가로 선택
                            const needed = 4 - candidatesWithMinCount.length;
                            const nextMinCount = sortedByCount.find(p => (playerPlayCount[p.userId] || 0) > minCount);
                            if (nextMinCount) {
                                ...
                                availablePlayers = [...candidatesWithMinCount, ...candidatesWithNextMinCount.slice(0, needed)].slice(0, 4);
                            } else {
                                // 그래도 부족하면 전체에서 선택
                                availablePlayers = sortedByCount.slice(0, 4);
                            }
                        ...
                        // 7,8 경기: 1,2 경기와 비슷하게 최강자, 최약자끼리 경기하되 중복 방지
                        // 참여 안한 사람 우선 포함
                        const notPlayed = remainingPlayers.filter(p => !playedPlayers.has(p.userId));
                ...
                availablePlayers.sort((a, b) => {
                    ...
                    // 그룹 모드: 코트가 이미 점수별로 나눠져 있으므로 완전 랜덤
                    ...
                    for (let attempt = 0; attempt < maxAttempts && !found; attempt++) {
                        ...
                        if (teamMode === 'balanced') {
                            ...
                            if (matchNum === 1 || matchNum === 2) {
                                ...
                                // 코트 번호에 따라 상위/하위 배정 (홀수 코트: 상위, 짝수 코트: 하위)
                                if (c % 2 === 1) {
                                    // 홀수 코트: 상위 4명 중에서 잘하는 사람들끼리 vs 잘하는 사람들끼리
                                    // 1경기와 2경기 중복 방지
                                    const forbiddenCombinations = new Set();
                                    if (matchNum === 2) {
                                        for (const prev of previousMatchConfigs) {
                                            if (prev.matchNum === 1 && prev.court === c) {
                                    ...
                                    // 최강+차강 vs 차강+차약
                                    const teamA = [sorted[0], sorted[1]].map(p => p.userId).sort().join(',');
                                    const teamB = [sorted[2], sorted[3]].map(p => p.userId).sort().join(',');
                                    const comboKey = `${teamA}|${teamB}`;
                                    ...
                                    // 짝수 코트: 하위 4명 중에서 못하는 사람들끼리 vs 못하는 사람들끼리
                                    // 하위 4명 기준: sorted[0] = 하위 1위, sorted[1] = 하위 2위, sorted[2] = 하위 3위, sorted[3] = 하위 4위
                                    // 1경기와 2경기 중복 방지
                                    const forbiddenCombinations = new Set();
                                    if (matchNum === 2) {
                                        for (const prev of previousMatchConfigs) {
                                    ...
                                    // 차약+최약 vs 차약+차강
                                    const teamA = [sorted[2], sorted[3]].map(p => p.userId).sort().join(',');
                                    const teamB = [sorted[0], sorted[1]].map(p => p.userId).sort().join(',');
                                    const comboKey = `${teamA}|${teamB}`;
                                    
                                    if (!forbiddenCombinations.has(comboKey)) {
                                        teamConfigs.push({ 
                                            teamA: [sorted[2], sorted[3]].map(p => p.userId), 
                                            teamB: [sorted[0], sorted[1]].map(p => p.userId) 
                                        });
                                    } else {
                                        // 중복이면 다른 조합 사용
                                        teamConfigs.push({ 
                                ...
                                for (const prev of previousMatchConfigs) {
                                    if (prev.matchNum === 1 || prev.matchNum === 2) {
                                        // teamAIds와 teamBIds는 이미 정렬된 문자열이므로 그대로 사용
                                        const prevTeamA = prev.teamAIds;
                                        const prevTeamB = prev.teamBIds;
                                        forbiddenCombinations.add(`${prevTeamA}|${prevTeamB}`);
                                        forbiddenCombinations.add(`${prevTeamB}|${prevTeamA}`);
                                ...
                                // 1,2 경기와 비슷한 조합 (최강+차강 vs 차약+최약) 시도
                                const similarCombination = [
                                    [[sorted[0], sorted[1]], [sorted[2], sorted[3]]]
                                ];
                                
                                for (const combo of similarCombination) {
                                    const teamA = combo[0].map(p => p.userId).sort().join(',');
                                    const teamB = combo[1].map(p => p.userId).sort().join(',');
                                    const comboKey = `${teamA}|${teamB}`;
                        ...
                        for (const config of teamConfigs) {
                            ...
                            for (const prev of previousMatchConfigs) {
                                ...
                                // 4경기일 때: 3경기에서 같은 팀이었던 조합 방지 (완전 제외)
                                if (matchNum === 4) {
                                    let hasSameTeammateCombo = false;
                                    for (const prev of previousMatchConfigs) {
                                        if (prev.matchNum === 3) {
                                    ...
                                    // 같은 팀원 조합이 있으면 완전히 제외 (매우 큰 패널티)
                                    if (hasSameTeammateCombo) {
                                        duplicateCount += 1000; // 완전 제외를 위한 매우 큰 패널티
                                    }
                                }
                                
                                // 7경기일 때: 3,4경기에서 같은 팀이었던 조합 방지 (완전 제외)
                                if (matchNum === 7) {
                                    let hasSameTeammateCombo = false;
                                    for (const prev of previousMatchConfigs) {
                                        if (prev.matchNum === 3 || prev.matchNum === 4) {
                                ...
                                // 전체 이력에서 같은 팀원 반복 체크 (4경기, 7경기, 8경기가 아닌 경우에만)
                                if (matchNum !== 4 && matchNum !== 7 && matchNum !== 8) {
                            ...
                            // 밸런스 모드와 그룹 모드에서는 가장 중복이 적은 조합 선택
                        ...
                        if (teamMode === 'balanced') {
                            ...
                            if (matchNum === 1 || matchNum === 2) {
                                ...
                                if (c % 2 === 1) {
                                    ...
                                    if (!forbiddenCombinations.has(comboKey)) {
                                        selectedTeamA = [sorted[0], sorted[1]]; // 최강+차강
                                        selectedTeamB = [sorted[2], sorted[3]]; // 차강+차약
                                    } else {
                                        // 중복이면 다른 조합 사용
                                        selectedTeamA = [sorted[0], sorted[2]]; // 최강+차강
                                        selectedTeamB = [sorted[1], sorted[3]]; // 차강+차약
                                    }
                                } else {
                                    // 짝수 코트: 하위 4명 중에서 못하는 사람들끼리 vs 못하는 사람들끼리
                                    // 하위 4명 기준: sorted[0] = 하위 1위, sorted[1] = 하위 2위, sorted[2] = 하위 3위, sorted[3] = 하위 4위
                                    ...
                                    if (!forbiddenCombinations.has(comboKey)) {
                                        selectedTeamA = [sorted[2], sorted[3]]; // 하위 3위+하위 4위 (차약+최약)
                                        selectedTeamB = [sorted[0], sorted[1]]; // 하위 1위+하위 2위 (차약+차강)
                                    } else {
                                        // 중복이면 다른 조합 사용
                                        selectedTeamA = [sorted[2], sorted[3]]; // 하위 3위+하위 4위 (차약+최약)
                                        selectedTeamB = [sorted[1], sorted[2]]; // 하위 2위+하위 3위 (차강+차약)
                                    }
                                }
                            } else if (matchNum === 3 || matchNum === 4) {
                                ...
                                if (courtCount >= 2) {
                                    const candidateTeamA = [sorted[0], sorted[1]].map(p => p.userId).sort();
                                    const candidateTeamB = [sorted[2], sorted[3]].map(p => p.userId).sort();
                                    
                                    // 3,4경기와 같은 팀원 조합인지 확인
                                    for (const prev of previousMatchConfigs) {
                                        if (prev.matchNum === 3 || prev.matchNum === 4) {
                                            const prevTeamA = prev.teamAIds.split(',').sort();
                                            const prevTeamB = prev.teamBIds.split(',').sort();
                                            ...
                                            const prevTeamASet = new Set(prevTeamA);
                                            const prevTeamBSet = new Set(prevTeamB);
                                            const candidateTeamASet = new Set(candidateTeamA);
                                            const candidateTeamBSet = new Set(candidateTeamB);
                                ...
                                // 7경기 정보를 previousMatchConfigs에 저장
                                const teamAIds = selectedTeamA.map(p => p.userId).sort().join(',');
                                const teamBIds = selectedTeamB.map(p => p.userId).sort().join(',');
                                previousMatchConfigs.push({ 
                                    teamAIds, 
                                ...
                                if (courtCount >= 2) {
                                    ...
                                    for (const prev of previousMatchConfigs) {
                                        if (prev.matchNum === 3 || prev.matchNum === 4 || prev.matchNum === 7) {
                                            ...
                                            const prevTeamASet = new Set(prevTeamA);
                                            const prevTeamBSet = new Set(prevTeamB);
                                            const candidateTeamASet1 = new Set(candidateTeamA1);
                                            const candidateTeamBSet1 = new Set(candidateTeamB1);
                                            
                                            // 3,4,7경기 Team A/B와 8경기 후보 Team A/B가 같은지 확인
                                            if ((prevTeamASet.size === candidateTeamASet1.size && 
                                                [...prevTeamASet].every(id => candidateTeamASet1.has(id))) ||
                                                (prevTeamASet.size === candidateTeamBSet1.size && 
                                                [...prevTeamASet].every(id => candidateTeamBSet1.has(id))) ||
                                                (prevTeamBSet.size === candidateTeamASet1.size && 
                                                [...prevTeamBSet].every(id => candidateTeamASet1.has(id))) ||
                                                (prevTeamBSet.size === candidateTeamBSet1.size && 
                                                [...prevTeamBSet].every(id => candidateTeamBSet1.has(id)))) {
                                                hasSameTeammateCombo = true;
                                                break;
                                            }
                                        }
                                    ...
                                    if (hasSameTeammateCombo) {
                                        // 3,4,7경기와 같은 팀원 조합이면 다른 조합 사용
                                        selectedTeamA = [sorted[0], sorted[2]]; // 하위 1위+하위 3위
                                        selectedTeamB = [sorted[1], sorted[3]]; // 하위 2위+하위 4위
                                    } else {
                                        selectedTeamA = [sorted[2], sorted[3]]; // 못하는 사람들 (하위 3위+하위 4위)
                            ...
                            selectedTeamA = [shuffledPlayers[0], shuffledPlayers[1]];
                            selectedTeamB = [shuffledPlayers[2], shuffledPlayers[3]];
                        } else {
                    ...
                    if (teamMode === 'balanced') {
                        ...
                        if (matchNum === 1 || matchNum === 2) {
                            ...
                            if (matchNum === 3) {
                                ...
                                // 3경기 정보를 previousMatchConfigs에 저장
                                const teamAIds = selectedTeamA.map(p => p.userId).sort().join(',');
                                const teamBIds = selectedTeamB.map(p => p.userId).sort().join(',');
                                previousMatchConfigs.push({ 
                                    teamAIds, 
                                ...
                                for (const prev of previousMatchConfigs) {
                                    if (prev.matchNum === 3) {
                                        ...
                                        // 3경기 Team A와 4경기 후보 Team A가 같은 팀원 조합인지 확인
                                        if (prevTeamASet.size === candidateTeamASet.size && 
                                            [...prevTeamASet].every(id => candidateTeamASet.has(id))) {
                                            hasSameTeammateCombo = true;
                                            break;
                                        }
                                        // 3경기 Team A와 4경기 후보 Team B가 같은 팀원 조합인지 확인
                                        if (prevTeamASet.size === candidateTeamBSet.size && 
                                            [...prevTeamASet].every(id => candidateTeamBSet.has(id))) {
                                            hasSameTeammateCombo = true;
                                            break;
                                        }
                                        // 3경기 Team B와 4경기 후보 Team A가 같은 팀원 조합인지 확인
                                        if (prevTeamBSet.size === candidateTeamASet.size && 
                                            [...prevTeamBSet].every(id => candidateTeamASet.has(id))) {
                                            hasSameTeammateCombo = true;
                                            break;
                                        }
                                        // 3경기 Team B와 4경기 후보 Team B가 같은 팀원 조합인지 확인
                                        if (prevTeamBSet.size === candidateTeamBSet.size && 
                                            [...prevTeamBSet].every(id => candidateTeamBSet.has(id))) {
                                            hasSameTeammateCombo = true;
                                            break;
                                        }
                            ...
                            for (const prev of previousMatchConfigs) {
                                if (prev.matchNum === 7) {
                                    ...
                                    // 7경기 Team A와 8경기 후보 Team A가 같은 팀원 조합인지 확인
                                    if (prevTeamASet.size === candidateTeamASet.size && 
                                        [...prevTeamASet].every(id => candidateTeamASet.has(id))) {
                                        hasSameTeammateCombo = true;
                                        break;
                                    }
                                    // 7경기 Team A와 8경기 후보 Team B가 같은 팀원 조합인지 확인
                                    if (prevTeamASet.size === candidateTeamBSet.size && 
                                        [...prevTeamASet].every(id => candidateTeamBSet.has(id))) {
                                        hasSameTeammateCombo = true;
                                        break;
                                    }
                                    // 7경기 Team B와 8경기 후보 Team A가 같은 팀원 조합인지 확인
                                    if (prevTeamBSet.size === candidateTeamASet.size && 
                                        [...prevTeamBSet].every(id => candidateTeamASet.has(id))) {
                                        hasSameTeammateCombo = true;
                                        break;
                                    }
                                    // 7경기 Team B와 8경기 후보 Team B가 같은 팀원 조합인지 확인
                                    if (prevTeamBSet.size === candidateTeamBSet.size && 
                                        [...prevTeamBSet].every(id => candidateTeamBSet.has(id))) {
                                        hasSameTeammateCombo = true;
                                        break;
                        ...
                        selectedTeamA = [shuffled[0], shuffled[1]];
                        selectedTeamB = [shuffled[2], shuffled[3]];
                    } else {
                        const p = pairingPatterns[(r - 1) % pairingPatterns.length];
                        selectedTeamA = [fourPlayers[p[0]], fourPlayers[p[1]]];
                        selectedTeamB = [fourPlayers[p[2]], fourPlayers[p[3]]];
                ...
                // 5,6,1,2,7,8 경기 참여 플레이어 기록 (3,4 경기에서 사용)
                if (teamMode === 'balanced' && (courtMatchNumber === 5 || courtMatchNumber === 6 || courtMatchNumber === 1 || courtMatchNumber === 2 || courtMatchNumber === 7 || courtMatchNumber === 8)) {
                    fourPlayers.forEach(player => {
                        playedPlayers.add(player.userId);
                    });
                }
                ...
                previousMatchConfigs.push({ 
                    teamAIds: teamAIdsStr, 
                    teamBIds: teamBIdsStr,
                    matchNum: courtMatchNumber 
                });
            ...
            if (teamMode === 'balanced') {
                ...
                // 그룹 모드: 코트가 이미 점수별로 나눠졌으므로 완전 랜덤하게 섞기
                sortedAllPlayers = [...courtPlayerList].sort(() => Math.random() - 0.5);
            } else {
                // 랜덤 모드: 랜덤하게 섞기
                sortedAllPlayers = [...courtPlayerList].sort(() => Math.random() - 0.5);
                ...
                if (candidatesWithMinCount.length >= 4) {
                    ...
                    for (let attempt = 0; attempt < maxAttempts && !found; attempt++) {
                        ...
                        if (teamMode === 'balanced') {
                            // 밸런스 모드: 경기 번호에 따라 조합 결정
                            courtMatchNumber2++;
                            const matchNum = courtMatchNumber2;
                            ...
                            if (matchNum === 1 || matchNum === 2) {
                                ...
                                // 7경기: 코트가 2개 이상이면 잘하는 사람들끼리, 못하는 사람들끼리 (1,2 경기와 같은 조합이지만 중복 방지)
                                // 코트가 1개면 밸런스 조합
                                const forbiddenCombinations = new Set();
                                // 1,2 경기 조합 금지
                                for (const prev of previousMatchConfigs) {
                                ...
                                if (courtCount >= 2) {
                                    // 코트가 2개 이상: 잘하는 사람들끼리, 못하는 사람들끼리 (1,2 경기와 같은 조합이지만 중복 방지)
                                    const similarCombination = [
                                        [[sorted[0], sorted[1]], [sorted[2], sorted[3]]] // 잘하는 사람들 vs 못하는 사람들
                                    ];
                                    ...
                                    // 코트가 1개: 밸런스 조합 (1,2 경기와 중복되지 않는)
                                    const balanceCombinations = [
                                        [[sorted[0], sorted[3]], [sorted[1], sorted[2]]], // 최강+최약 vs 차강+차약
                                        [[sorted[0], sorted[2]], [sorted[1], sorted[3]]]  // 최강+차약 vs 차강+최약
                                    ];
                                    
                                    for (const combo of balanceCombinations) {
                                        const teamA = combo[0].map(p => p.userId).sort().join(',');
                                        const teamB = combo[1].map(p => p.userId).sort().join(',');
                                        const comboKey = `${teamA}|${teamB}`;
                                ...
                                if (courtCount >= 2) {
                                    // 코트가 2개 이상: 가능한 조합들 시도 (1,2,7 경기와 중복되지 않는)
                                    const possibleCombos = [
                                        [[sorted[0], sorted[1]], [sorted[2], sorted[3]]], // 최강+차강 vs 차약+최약
                                        [[sorted[0], sorted[3]], [sorted[1], sorted[2]]], // 최강+최약 vs 차강+차약
                                        [[sorted[0], sorted[2]], [sorted[1], sorted[3]]]  // 최강+차약 vs 차강+최약
                                    ...
                                    // 코트가 1개: 밸런스 조합 (1,2,7 경기와 중복되지 않는)
                                    const balanceCombinations = [
                                        [[sorted[0], sorted[3]], [sorted[1], sorted[2]]], // 최강+최약 vs 차강+차약
                                        [[sorted[0], sorted[2]], [sorted[1], sorted[3]]]  // 최강+차약 vs 차강+최약
                                    ];
...
async function saveMatchScore(match, scoreA, scoreB) {
    try {
        ...
        await recordGameResult(`${match.id}_A`, {
            ...
            gameStartTime: gameStartTime, // 시간당 8경기 간격으로 계산된 시간
            gameEndTime: gameEndTime,     // 시간당 8경기 간격으로 계산된 시간
...
async function saveManualMatchSchedule() {
    try {
        ...
        // 시작 시간에서 1시간 뒤 종료 시간 자동 계산
        const [startHour, startMinute] = startTime.split(':').map(Number);
        const startTotalMinutes = startHour * 60 + startMinute;
        const endTotalMinutes = startTotalMinutes + 60; // 1시간 추가
