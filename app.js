...
function createBalancedRotationSchedule(players, courtCount, rounds) {
    ...
    // 그룹 수 결정: 플레이어 수에 따라 2그룹 또는 3그룹
    let groupCount = 2;
    if (totalPlayers >= 12) {
        groupCount = 3; // 12명 이상이면 3그룹
    }
...
function buildMatchSchedule(players, courtCount, rounds, playerCourtMap = {}, teamMode = 'random') {
    ...
    // 랜덤 모드: 기존 로직 사용
        ...
        for (let c = 1; c <= courtCount; c++) {
            console.log(`🎯 코트 ${c} 경기 생성 시작...`);
            
            for (let r = 1; r <= rounds; r++) {
                console.log(`🎯 코트 ${c}, 경기 ${r}/${rounds} 생성 중...`);
                ...
                if (availableThisMatch.length < 4) {
                    // 4명이 안 되면 경기 생성 중단
                    console.warn(`⚠️ 코트 ${c}, 경기 ${r} 생성 실패: 플레이어 부족 (${availableThisMatch.length}명)`);
                    break;
                }
                ...
                if (!found) {
                    if (!fourPlayers || fourPlayers.length < 4) {
                        console.warn(`⚠️ 코트 ${c}, 경기 ${r} 생성 실패: fourPlayers 없음`);
                        break;
                    }
