...
// 예약 마감 시간 확인 및 대진표 자동 생성 (1분마다 자동 실행)
...
// 특정 시간 슬롯의 예약 처리 및 대진표 생성 (자동/수동 호출 가능)
...
// 자동 예약 처리 시작 (1분마다 마감 시간 확인 및 대진표 자동 생성)
function startAutoProcessing() {
    ...
    console.log('✅ 자동 예약 처리 시작 (1분마다 마감 시간 확인 및 대진표 자동 생성)');
}
...
function createTournamentBracket(teams) {
    ...
    while (expectedWinners > 1) {
        ...
        // 이전 라운드의 승자들을 대진표 구조에 맞게 다음 라운드로 배정
        // 매치 인덱스를 기반으로 정확한 위치에 배정
        // 예: 16강 매치 0,1 → 8강 매치 0, 16강 매치 2,3 → 8강 매치 1
        const winnersByNextMatch = {}; // { nextMatchIndex: [team1, team2] }
        ...
        prevRound.matches.forEach((match, matchIndex) => {
            ...
            if (winner) {
                // 다음 라운드의 매치 인덱스 계산: 매치 인덱스를 2로 나눈 몫
                const nextMatchIndex = Math.floor(matchIndex / 2);
                // 같은 쌍 내에서의 위치: 0이면 team1, 1이면 team2
                const positionInPair = matchIndex % 2;
                
                if (!winnersByNextMatch[nextMatchIndex]) {
                    winnersByNextMatch[nextMatchIndex] = [null, null];
                }
                
                if (positionInPair === 0) {
                    winnersByNextMatch[nextMatchIndex][0] = winner; // team1
                } else {
                    winnersByNextMatch[nextMatchIndex][1] = winner; // team2
                }
            }
        });
        ...
        const nextRoundMatchCount = Math.ceil(actualWinners / 2);
        
        console.log(`[대진표 생성] 라운드 ${roundNumber}: 이전 라운드 매치 수=${prevRound.matches.length}, 승자 수=${actualWinners}, 다음 라운드 매치 수=${nextRoundMatchCount}, 부전승 승자 배정=${Object.keys(winnersByNextMatch).length}개 매치`);
        
        // 승자들을 두 팀씩 매칭 (부전승 없음)
        // 대진표 구조에 맞게 정확한 위치에 배정
        for (let i = 0; i < nextRoundMatchCount; i++) {
            ...
            // 부전승 승자가 있으면 정확한 위치에 배정
            if (winnersByNextMatch[i]) {
                match.team1 = winnersByNextMatch[i][0];
                match.team2 = winnersByNextMatch[i][1];
            }
...
function renderTournamentBracket(bracket, tournamentId) {
    ...
    for (let roundIndex = 0; roundIndex < totalRounds - 1; roundIndex++) {
        ...
        if (leftRoundMatches && leftRoundMatches.length > 0) {
            ...
            // 4강 첫 경기는 8강 팀1234의 중간에 위치하도록 조정
            if (roundIndex === 1 && leftRoundMatches.length === 2) {
                // 첫 번째 매치를 먼저 렌더링
                const firstMatch = leftRoundMatches[0];
                ...
                // 두 번째 매치 렌더링
                const secondMatch = leftRoundMatches[1];
    ...
    if (finalMatch) {
        ...
        bracketHTML += `
            <div class="bracket-match-split">
                <div class="match-team-split team1 empty">
                    <div class="team-name-split empty-team">대기 중</div>
                </div>
    ...
    try {
        ...
        if (roundIndex < bracket.rounds.length - 1) {
            ...
            if (roundIndex === totalRounds - 2) {
                ...
                if (isLeftSide) {
                    // 왼쪽 브래킷: 내부에서만 진행
                    const leftMatchIndex = currentMatchIndex;
                    nextMatchIndex = Math.floor(leftMatchIndex / 2);
                } else {
                    // 오른쪽 브래킷: 내부에서만 진행
                    const rightMatchIndex = currentMatchIndex - halfPoint;
                    const leftMatchesInNextRound = Math.ceil(halfPoint / Math.pow(2, roundIndex + 1));
                    nextMatchIndex = leftMatchesInNextRound + Math.floor(rightMatchIndex / 2);
                }
